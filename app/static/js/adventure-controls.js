...
(function partyCardRefresh() {
    function paint(member) {
        ...
        // frame, the weight belongs in the panel. It gets its own row
        // (.party-status-line), whose reserved height the rail budget already
        // pays for whether or not a marker shows. A badge in the header row
        // was tried first and rejected: it measured fine but clipped in the
        // browser once a real (long) class name sat next to it. The DEX
        // penalty moves to a hover title so the badge text itself stays short.
