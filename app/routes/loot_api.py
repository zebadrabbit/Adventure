...
from app.inventory.utils import load_inventory, dump_inventory, add_item, can_add_item
...
def claim_loot(loot_id: int):
    ...
    if not inst or row.seed != inst.seed:
        return jsonify({'error': 'wrong dungeon', 'expected_seed': (inst.seed if inst else None), 'row_seed': row.seed, 'inst_id': inst_id}), 400
    ...
    if requested_char_id is not None:
        ...
        if party:
            ...
            if target_char.name not in names:
                return jsonify({'error': 'character not in active party', 'character': target_char.name, 'party': list(names)}), 403
    else:
        ...
        if party:
            names = [m.get('name') for m in party if isinstance(m, dict)]
            if names:
                target_char = Character.query.filter_by(user_id=effective_user_id).filter(Character.name.in_(names)).first()
        if not target_char:
    ...
    return jsonify({
        'claimed': True,
        'item': {'slug': item.slug if item else None, 'name': item.name if item else None},
