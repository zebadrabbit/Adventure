...
import os, sys, re, json, getpass, argparse
...
class C:
    ...
    def wrap(msg, color): return f"{color}{msg}{C.RESET}"

def supports_color(): return sys.stdout.isatty() and os.getenv('NO_COLOR') is None
def c(msg, color): return C.wrap(msg, color) if supports_color() else msg
...
if ENV_FILE.exists():
    for line in ENV_FILE.read_text().splitlines():
        ...
        if '=' in line:
            k,v=line.split('=',1); existing_env[k.strip()]=v.strip()
...
parser.add_argument('--quiet', action='store_true', help='Minimal output (overrides verbose unless JSON).')
parser.add_argument('--verbose', action='store_true', help='Extra informational output.')
parser.add_argument('--log-level', choices=['debug','info','warn','error','silent'], help='Explicit log verbosity (overrides --quiet/--verbose).')
...
if args.log_level: LOG_LEVEL = args.log_level
    ...
    if args.quiet: LOG_LEVEL='silent'
    elif args.verbose: LOG_LEVEL='debug'
    else: LOG_LEVEL='info'

_ORDER=['debug','info','warn','error','silent']
def _idx(l): return _ORDER.index(l)
def log_enabled(level:str)->bool:
    if EMIT_JSON or LOG_LEVEL=='silent': return False
    return _idx(level) >= _idx(LOG_LEVEL)
def log(msg, level='info', color=None):
    if log_enabled(level): print(c(msg,color) if color else msg)
...
def prompt(prompt_text: str, default: Optional[str]=None, secret: bool=False, validator=None):
    ...
    while True:
        ...
        raw = getpass.getpass(c(prompt_text+suffix+': ', C.CYAN)) if secret else input(c(prompt_text+suffix+': ', C.CYAN))
        if not raw and default is not None: raw = default
        if validator:
            ok, err = validator(raw)
            if not ok:
                print(c(f"  ! {err}", C.RED)); continue
        return raw

if log_enabled('info'):
    log("\nAdventure MUD Setup", 'info', C.BOLD + C.MAGENTA)
    log("──────────────────────", 'info', C.MAGENTA)
    log("This script will help you configure the environment and bootstrap the database.", 'info', C.GRAY)
    log('', 'info')

INSTANCE_DIR.mkdir(exist_ok=True)

secret_key = prompt("Secret key (used for sessions & CSRF)", DEFAULT_SECRET, validator=lambda v: (len(v)>=8,'Must be at least 8 chars'))
secret_key_generated = False
PLACEHOLDER_VALUES = { 'dev-secret-change-me','changeme','change-me','secret','placeholder' }
...
if args.no_admin:
    create_admin='n'; admin_username=''; admin_password=''
else:
    if NON_INTERACTIVE and args.admin_username:
        admin_username = args.admin_username
        if args.admin_password: admin_password = args.admin_password
        elif args.generate_admin_password:
            import secrets; admin_password = secrets.token_urlsafe(16)
        else: admin_password=''
        create_admin='y'
    else:
...
new_env.update({'SECRET_KEY': secret_key,'DATABASE_URL': database_url,'CORS_ALLOWED_ORIGINS': cors,'ADMIN_USERNAME': admin_username if create_admin=='y' else ''})
ENV_FILE.write_text('\n'.join(f"{k}={v}" for k,v in new_env.items() if v is not None)+'\n')
log(f"✓ Wrote {ENV_FILE.relative_to(PROJECT_ROOT)}", 'info', C.GREEN)

os.environ.setdefault('FLASK_ENV','development'); os.environ.setdefault('PYTHONPATH', str(PROJECT_ROOT))
try:
    import app as app_module  # noqa: F401
    from app import db, create_app  # type: ignore
    from app.models.models import User
    flask_app = getattr(app_module,'app',None)
    if flask_app is None or not hasattr(flask_app,'app_context'): flask_app = create_app()
except Exception as e:
    log("Failed to import app – ensure dependencies installed (pip install -r requirements.txt).", 'error', C.RED)
    log(str(e), 'error', C.RED); sys.exit(1)

try:
    import importlib, app.server  # noqa: F401
except Exception as e:
    log(f"Warning: could not run runtime migrations automatically: {e}", 'warn', C.YELLOW)
    ...
    if create_admin.lower()=='y':
        ...
        if existing:
            ...
            if rotate.lower()=='y' and admin_password:
                existing.password = generate_password_hash(admin_password); db.session.commit(); log("  ✓ Password updated", 'info', C.GREEN)
        else:
            u = User(username=admin_username, password=generate_password_hash(admin_password), role='admin')
            db.session.add(u); db.session.commit(); created_admin=True; log(f"✓ Created admin user '{admin_username}'", 'info', C.GREEN)
...
if log_enabled('info'):
    log('', 'info'); log("Setup Summary", 'info', C.BOLD + C.MAGENTA); log("──────────────", 'info', C.MAGENTA)
    log(f"Database URL: {database_url}", 'info', C.CYAN)
    log(f"Secret Key: {'(hidden)' if summary['secret_key_set'] else secret_key}", 'info', C.CYAN)
    log(f"CORS Origins: {cors}", 'info', C.CYAN)
    if create_admin.lower()=='y':
        if created_admin: log(f"Admin Credentials: {admin_username} / {admin_password}", 'warn', C.YELLOW)
        else: log("Admin Credentials: (unchanged)", 'info', C.YELLOW)
    log('', 'info'); log("Next steps:", 'info', C.BOLD)
    log("  1. (Optional) Create a virtualenv & install dependencies: pip install -r requirements.txt", 'info', C.GRAY)
    log("  2. Run the server: python run.py", 'info', C.GRAY)
    log("  3. Login with the admin account and explore!", 'info', C.GRAY)
    log('', 'info'); log("Done. Happy adventuring!", 'info', C.GREEN)
