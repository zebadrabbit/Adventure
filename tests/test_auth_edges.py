...
def test_dashboard_password_change_flow(auth_client):
    ...
    auth_client.post('/login', data={'username':'tester','password':'initialPW'}, follow_redirects=True)
    r = auth_client.post('/dashboard', data={'form':'change_password','current_password':'initialPW','new_password':new_password,'confirm_password':new_password}, follow_redirects=True)
    assert r.status_code == 200
