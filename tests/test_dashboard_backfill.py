...
def test_dashboard_backfills_missing_class(auth_client, ensure_items):
    ...
    with auth_client.application.app_context():
        ...
        c1 = Character(
            user_id=user.id,
            name="NoClassDruid",
            stats=_mk_stats(wis=14, int=9, dex=9, str=8),
            items=json.dumps(["herbal-pouch"]),
            gear=json.dumps([]),
        )
        ...
        c2 = Character(
            user_id=user.id,
            name="NoClassRanger",
            stats=_mk_stats(dex=14, wis=12, str=9, int=8),
            items=json.dumps(["hunting-bow"]),
            gear=json.dumps([]),
        )
        ...
        c3 = Character(
            user_id=user.id,
            name="NoClassFighter",
            stats=_mk_stats(str=16, dex=10, int=9, wis=8),
            items=json.dumps([]),
            gear=json.dumps([]),
        )
        ...
        c4 = Character(
            user_id=user.id,
            name="NoClassMage",
            stats=_mk_stats(int=16, str=8, dex=9, wis=10),
            items=json.dumps([]),
            gear=json.dumps([]),
        )
        ...
        c5 = Character(
            user_id=user.id,
            name="NoClassRogue",
            stats=_mk_stats(dex=16, str=8, int=9, wis=7),
            items=json.dumps([]),
            gear=json.dumps([]),
        )
    ...
    with auth_client.application.app_context():
        updated = {
            c.name: json.loads(Character.query.filter_by(name=c.name).first().stats) for c in [c1, c2, c3, c4, c5]
        }
