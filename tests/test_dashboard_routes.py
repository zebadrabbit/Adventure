...
def client_authed(client, test_app):
    ...
    client.post(
        "/login",
        data={"username": "dashuser", "password": "pw123456"},
...
def test_character_creation_flow(client_authed):
    resp = client_authed.post(
        "/dashboard",
        data={"name": "Auron", "char_class": "fighter"},
        follow_redirects=True,
    )
...
def test_change_password_success(client_authed):
    ...
    login2 = client_authed.post(
        "/login",
        data={"username": "dashuser", "password": "newstrong"},
        follow_redirects=True,
    )
...
def test_start_adventure_party_validation(client_authed):
    ...
    resp2 = client_authed.post(
        "/dashboard",
        data={"form": "start_adventure", "party_ids": str(first_id)},
