...
def setup_user_flags():
    ...
    with app.app_context():
        ...
        if not admin:
            admin = User(username='admin_actor', password=generate_password_hash('pass'), role='admin')
            db.session.add(admin)
        target = User.query.filter_by(username='flagged_user').first()
        if not target:
            target = User(username='flagged_user', password=generate_password_hash('pass'), role='user')
...
def test_rate_limit_auto_mute(setup_user_flags, monkeypatch):
    ...
    class DummyPlayer: role='user'; username='flagged_user'
    ...
    player_c.get_received(); admin_c.get_received()
    player_c.emit('lobby_chat_message', {'message': 'after-mute'})
    rec_admin = admin_c.get_received()
    muted_msgs = [p for p in rec_admin if p['name']=='lobby_chat_message' and p['args'] and p['args'][0].get('message')=='after-mute']
