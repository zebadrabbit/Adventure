...
def test_register_and_redirect_dashboard(client, test_app):
    resp = client.post('/register', data={'username': 'newuser', 'password': 'secret123'}, follow_redirects=False)
...
def test_login_success_and_logout(client, test_app):
    with test_app.app_context():
        ...
        db.session.commit()
    # Login
    resp = client.post('/login', data={'username': 'loginuser', 'password': 'pass123'}, follow_redirects=False)
    assert resp.status_code in (302, 303)
...
def test_login_failure(client):
    resp = client.post('/login', data={'username': 'unknown', 'password': 'nope'}, follow_redirects=True)
