...
def test_autofilled_character_starts_at_full_mana(test_app):
    with test_app.app_context():
        from app.routes.dashboard_helpers import handle_autofill

        u = User(username=f"mp-autofill-{uuid.uuid4().hex[:8]}", email=f"{uuid.uuid4().hex[:8]}@test.local")
        u.set_password("pw")
        db.session.add(u)
        db.session.commit()
        ...
        for char in created:
            stats = json.loads(char.stats)
            intel = int(stats.get("int", 10))
            mana_max = 20 + intel * 2
            assert stats["mana"] == mana_max, (
                f"{char.name}: expected fresh character's stats['mana'] to equal computed "
                f"mana_max ({mana_max}), got {stats['mana']}"
            )
