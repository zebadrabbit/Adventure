import pytest
pytest.skip("Legacy dungeon API test skipped after refactor", allow_module_level=True)
import json
...
def test_state_endpoint(auth_client, test_app):
    inst = ensure_instance(test_app)
    r = auth_client.get('/api/dungeon/state')
    assert r.status_code == 200
    data = r.get_json()
    assert 'pos' in data and 'exits' in data and 'desc' in data
...
def test_seed_determinism(auth_client, test_app):
    ...
    g1 = r1.get_json()['grid']
    g2 = r2.get_json()['grid']
    # Grids should be equal for same seed
    assert g1 == g2
