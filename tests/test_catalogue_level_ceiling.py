...
# A private family, so these fixtures are isolated from the real catalogue.
# Using a real family name made the tests pass on an unseeded database and fail
# on a seeded one -- exactly the order-dependence documented in docs/TESTING.md.
FAMILY = "_ceiltest"
...
def _catalogue():
    ...
    specs = [
        ("_ceil_rat", FAMILY, 1, 3, False, "common"),
        ("_ceil_wolf", FAMILY, 4, 9, False, "common"),
        ("_ceil_wyrm", FAMILY, 10, 14, False, "uncommon"),
        ("_ceil_titan", FAMILY, 15, 18, False, "rare"),
        ("_ceil_overlord", FAMILY, 18, 18, True, "boss"),
    ]
...
def test_within_the_catalogue_is_unchanged():
    pool = spawn_service._eligible_monsters(5, family=FAMILY)
...
def test_above_the_ceiling_falls_back_to_the_deepest_band():
    ...
    pool = spawn_service._eligible_monsters(40, family=FAMILY, include_boss=True)
...
def test_choosing_a_monster_far_above_the_ceiling_succeeds():
    monster = spawn_service.choose_monster(45, family=FAMILY)
...
def test_the_fallback_scales_to_the_requested_level():
    ...
    low = spawn_service.choose_monster(18, family=FAMILY)
    spawn_service.clear_cache()
    high = spawn_service.choose_monster(45, family=FAMILY)
...
def test_the_fallback_does_not_fire_inside_the_catalogue():
    ...
    pool = spawn_service._eligible_monsters(12, family=FAMILY)
