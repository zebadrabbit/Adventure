...
def test_email_update_valid_and_clear(auth_client):
    r = auth_client.post('/dashboard', data={'form':'update_email','email':'user@example.com'}, follow_redirects=True)
...
def test_email_invalid(auth_client):
    r = auth_client.post('/dashboard', data={'form':'update_email','email':'not-an-email'}, follow_redirects=True)
    assert r.status_code == 200


def test_password_change_paths(auth_client):
    # wrong current password
    r = auth_client.post('/dashboard', data={'form':'change_password','current_password':'wrong','new_password':'newpass1','confirm_password':'newpass1'}, follow_redirects=True)
    assert r.status_code == 200
    # mismatch confirmation
    r2 = auth_client.post('/dashboard', data={'form':'change_password','current_password':'pass','new_password':'abcdef','confirm_password':'ghijkl'}, follow_redirects=True)
    assert r2.status_code == 200
    # too short
    r3 = auth_client.post('/dashboard', data={'form':'change_password','current_password':'pass','new_password':'xx','confirm_password':'xx'}, follow_redirects=True)
...
def test_start_adventure_errors_and_success(auth_client):
    ...
    r2 = auth_client.post('/dashboard', data={'form':'start_adventure','party_ids':['9999']}, follow_redirects=True)
    assert r2.status_code == 200
    # valid party
    ...
    r3 = auth_client.post('/dashboard', data={'form':'start_adventure','party_ids':ids}, follow_redirects=True)
