...
- `party_system_migration.sql` – Creates `party`, `party_member`, `party_buff`, and `party_shared_inventory` plus their indexes, then seeds one default party from the existing characters. **Historical:** `party_shared_inventory` was dropped on 2026-07-30 (alembic `e4a91c73b2d8`) — bags are per-character. This file is left as the record of what was applied at the time; running it fresh recreates a table nothing reads, which the alembic revision then drops again.
